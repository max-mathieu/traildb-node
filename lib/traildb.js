...
TrailDB.prototype.getTrailId = function (uuid) {
  const trailId = ref.alloc(ref.types.uint64);
  const err = lib.tdb_get_trail_id(this._db, uuidRaw(uuid), trailId);
  return err ? '' : trailId.deref();
};
//
// Exports
//
